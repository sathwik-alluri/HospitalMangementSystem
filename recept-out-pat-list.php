...
<li><a href="recept-out-pat-list.php" class="active">Out-Patient List</a></li>
</ul>
</li>
<li class="submenu">
...
if(isset($_GET['search']))
{
    if(isset($_GET['pname']))
    {
        $today=date("Y-m-d");
        $pname=$_GET['pname'];
        $s1="select * from patients where p_name like '%$pname%' and d_o_c='$today'";
    }
}
else{
    $today=date("Y-m-d");
$s1="select * from patients where d_o_c='$today'";
}
$q1=mysqli_query($con,$s1);
while($run=mysqli_fetch_assoc($q1))
{
    $visited=$run['visited'];
echo '<tr>
<td>'.$run['p_id'].'</td>
...
<td>'.$run['type'].'</td>';
if($visited==1)
 echo '<td><button class="btn btn-success" >Visited</button></td></tr>';
else 
echo '<td><button class="btn btn-danger">Not Visited</button></td></tr>';
