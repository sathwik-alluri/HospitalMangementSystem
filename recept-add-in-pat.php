...
<ul>
<li><a href="recept-add-in-pat.php" class="active">Add In-Patient</a></li>
...
<div class="col">
<h3 class="page-title">Add In Patient</h3>
</div>
</div>
</div>
...
if(isset($_GET['search']))
{
    ...
    {
        $pname=$_GET['pname'];
        $s1="select * from patients where p_name like'%$pname%' and p_id NOT IN(select p_id from inpatients)";
    }
}
else{
$s1="select * from patients where p_id NOT IN(select p_id from inpatients)";
}
$q2=mysqli_query($con,$s1);
while($run=mysqli_fetch_assoc($q2))
{
...
<td>'.$run['type'].'</td>
<td class="text-end">
<button type="button" name="add" pid69="'.$run['p_id'].'" pname69="'.$run['p_name'].'" class="btn btn-rounded btn-info" onclick="visit(this)">Add</button></td>
</tr>';
}
?>
</tbody>
...
<script>
   function visit(buttonElement) {
    var patientid = buttonElement.getAttribute("pid69");
    var patientName = buttonElement.getAttribute("pname69");
    if (confirm("Do you want to add "+patientName+" as INPATIENT ?")==true) {
        window.location.href = 'recept-add-in-pat.php?a='+patientid;
    }   
}
</script>
<?php
$sql='select imp_id from inpatients ORDER BY imp_id DESC LIMIT 1';
$run1=mysqli_query($con,$sql);
$run1=mysqli_fetch_assoc($run1);
$value2 = substr($run1['imp_id'],3);
$value2 = (int)$value2 + 1;
$value2 = 'IMP'.sprintf('%s',$value2);
$pid69=$_GET['a'];
$doc=date("Y-m-d");
if(isset($pid69)){
$s1="INSERT INTO `inpatients`(`imp_id`, `p_id`, `d_o_a`) VALUES ('$value2','$pid69','$doc')";
$run1=mysqli_query($con,$s1);
echo "<script>document.location='recept-add-in-pat.php'</script>";
}
