...
$pid=$_GET['a'];
$query="UPDATE patients SET visited=1 WHERE p_id='$pid'";
$rus=mysqli_query($con,$query);
$today=date("Y-m-d");
$dname=$_SESSION['doc_name'];
$dtype=$_SESSION['type'];
$res="select *  from patients where type='$dtype' and d_o_c='$today'";
...
<li><a href="doc-op-list.php" class="active">OP-List</a></li>
...
<div class="page-wrapper">
    ...
    <ul class="breadcrumb">
    <li class="breadcrumb-item"><a href="students.html">Patient</a></li>
    <li class="breadcrumb-item active">OP-Patients</li>
    </ul>
    </div>
    </div>
    </div>
...
if(isset($_GET['search']))
  {
      if(isset($_GET['pname']))
      {
          ...
          $res="select *  from patients where type='$dtype1' and d_o_c='$today' and p_name like '%$pname%'";
          $res=mysqli_query($con,$res);
      }
  }
...
if($visited==1)
 ...
 echo '<td><button data-patient="'.$run['p_name'].'" data-patientid="'.$run['p_id'].'" class="btn btn-success"" >Visited</button></td></tr>';
else 
echo '<td><button data-patient="'.$run['p_name'].'" data-patientid="'.$run['p_id'].'" class="btn btn-danger" onclick="visit(this)" >Not Visited</button></td></tr>';
}
...
<script>
   function visit(buttonElement) {
    ...
    var patientid = buttonElement.getAttribute("data-patientid");
    if (confirm("Do you want to update "+patientName+" as visited?")==true) {
        window.location.href = 'doc-op-list.php?a='+patientid;
    }    
}
