...
$pd=$_GET['a'];
$q="delete from inpatients where p_id='$pd'";
...
<li><a href="recept-in-pat-list.php" class="active">In-Patient List</a></li>
...
if(isset($_GET['search']))
{
    ...
    {
        $pname=$_GET['pname'];
        $s1="select * from patients where p_name like'%$pname%' and p_id IN(select p_id from inpatients)";
    }
}
else{
$s1="select * from patients where p_id IN(select p_id from inpatients)";
}
...
<td>'.$run['age'].'</td>
<td>'.$run['type'].'</td>
<td><button data-patient="'.$run['p_name'].'" data-patientid="'.$run['p_id'].'" class="btn btn-danger" onclick="visit(this)" >Discharge</button></td></tr>';
}
?>
</tbody>
</table>
<script>
   function visit(buttonElement) {
    ...
    if (confirm("Do you want to update "+patientName+" as Discharged?")==true) {
        window.location.href = 'recept-in-pat-list.php?a='+patientid;
    }    
}
...
<script type="text/javascript">
        ...
        function downloadCSVFile(csv_data) {
            ...
            // Download csv file
            temp_link.download = "lathahospital_InPatients.csv";
